...
public abstract class AMovieCache : ALoggable, IMovieCache {
  ...
  public IMoviesPage GetMoviesPage(RFilter filter, int startPage = DEFAULT_START_PAGE, int pageSize = DEFAULT_PAGE_SIZE) {
    ...
    try {
      _LockCache.EnterReadLock();
      IEnumerable<IMovie> FilteredMovies = _Items.FilterByName(filter.Name).FilterByDate(filter.AddedAfter).OrderedByName();
...
public static class MovieExtensions {
  ...
  public static IEnumerable<IMovie> FilterByDate(this IEnumerable<IMovie> movies, DateOnly addedDate) {
    if (addedDate == DateOnly.MinValue) {
      return movies;
    }
    return movies.Where(m => m.DateAdded >= addedDate);
  }
