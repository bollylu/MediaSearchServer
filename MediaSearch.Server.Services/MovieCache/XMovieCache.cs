...
public class XMovieCache : AMovieCache {
  ...
  public override Task Parse(CancellationToken token) {
    return base.Parse(_FetchFiles(), CancellationToken.None);
  }

  public static XMovieCache Instance() {
    XMovieCache RetVal = new XMovieCache();
    RetVal.Parse(CancellationToken.None);
    return RetVal;
  }
}
